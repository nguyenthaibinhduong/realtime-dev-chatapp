...
function UserBadge({ user, onRemove }: { user: User; onRemove: () => void }) {
  return (
    <Badge
      variant="secondary"
...
export function ChannelDialog({ open, onOpenChange, type, onSuccess }: ChannelDialogProps) {
  ...
  const handleCreate = () => {
    ...
    if (type === "private" && selectedUsers.length === 0) {
      return toast({
        title: "Lỗi",
        description: "Vui lòng chọn ít nhất một thành viên cho kênh riêng tư",
        variant: "destructive",
      });
    }
    ...
    try {
      ...
      toast({
        title: "Thành công",
        description: `Tạo kênh ${type === "public" ? "công khai" : "riêng tư"} thành công`,
      });
      ...
      <DialogContent className="bg-gray-900 text-white border border-gray-700 w-full max-w-lg h-[600px] max-h-[90vh] flex flex-col">
            ...
            {selectedUsers.length > 0 && (
              <div className="flex flex-wrap gap-1 p-2 bg-gray-800 rounded-md border border-gray-600">
                {selectedUsers.map((user) => (
                  <UserBadge key={user.id} user={user} onRemove={() => handleRemoveUser(user.id)} />
                ))}
              </div>
            )}
                  ...
                  <ScrollArea className="h-full border border-gray-600 rounded-md bg-gray-800">
                    <div className="p-2 space-y-1">
                      {filteredSearchResults.map((user) => (
                        <UserListItem key={user.id} user={user} onAdd={() => handleAddUser(user)} />
                      ))}
                    </div>
                  </ScrollArea>
                ) : (
        ...
        <DialogFooter className="flex gap-2 pt-4 border-t border-gray-700">
          ...
          <Button
            ...
            disabled={!channelName.trim() || (type === "private" && selectedUsers.length === 0)}
            className="bg-blue-600 hover:bg-blue-700 text-white"
          >
            {isSearching && <Loader2 className="h-4 w-4 mr-2 animate-spin" />}
            {isSearching ? "Đang tạo..." : "Tạo kênh"}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
