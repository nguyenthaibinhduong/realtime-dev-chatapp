...
import { GitBranch, GitCommit, Folder, FileText, RefreshCw, ExternalLink, Play, Lock, Shield, Share2, ChevronLeft, ChevronRight } from "lucide-react";
...
export function CodeViewerDialog({
    ...
    const loadTree = useCallback(async () => {
        ...
        setLoadingTree(true);
        try {
            const tURL = repo.trees_url ? `${cleanTmpl(repo.trees_url)}/${encodeURIComponent(refParam)}?recursive=1` : "";
            ...
            {dialogPath &&
                dialogPath.split("/").map((seg, i, arr) => (
                    <span key={i}>
                        ...
                        <button className="hover:underline" onClick={() => setDialogPath(arr.slice(0, i + 1).join("/"))}>
                            {seg}
                        </button>
                    </span>
                ))}
    ...
    const handleDoShare = (type: "current" | "other") => {
        let channel_id = "";
        if (type === "current") channel_id = localStorage.getItem("selectedChannelId") || "";
        else channel_id = shareChannelId || "";

        const params = { repo, refParam, initialPath: selPath, installation_id, isShare: true, channel_id };
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <div className="flex-1 min-w-0">
                            ...
                            <div className="text-xs text-zinc-400 mt-1 flex items-center gap-2">
                                ...
                                <a href={repo.html_url} target="_blank" rel="noopener noreferrer" className="hover:underline text-zinc-300">
                                    {repo.html_url}
                                </a>
                                ...
                                <div className="flex-1 overflow-auto text-xs">
                                    ...
                                    {dirChildren.files.map((f) => (
                                        <button
                                            ...
                                            onClick={() => handleSelectFile(dialogPath ? `${dialogPath}/${f}` : f)}
                                    ...
                                    {!dirChildren.dirs.length && !dirChildren.files.length && <div className="p-3 text-zinc-500">Empty</div>}
                                </div>
                            )}
                        </div>
                    )}
...
const SkeletonLine = ({ width = "100%", height = "1rem", className = "" }) => (
    <div className={`bg-zinc-800 animate-pulse rounded ${className}`} style={{ width, height }} />
);
...
const RepoViewer: React.FC<RepoViewerProps> = ({ repo, onClose, installation_id }) => {
    ...
    const [viewBranchOfCommit, setViewBranchOfCommit] = useState<string | null>(null); // 👈 branch khi mở commit

    const refParam = useMemo(() => (mode === "branch" ? branch || defaultBranch : commitSha || ""), [mode, branch, commitSha, defaultBranch]);
    ...
    const loadCommitsForBranch = useCallback(
        async (page = 1, search = "") => {
            ...
            const data = await fetchJson(url, installation_id);
            let list: CommitItem[] = Array.isArray((data as any)?.data) ? (data as any).data : Array.isArray(data) ? (data as any) : [];
            ...
            if (searching) {
                ...
                list = list.filter((c: any) => {
                    ...
                    return sha.includes(s) || msg.includes(s) || login.includes(s) || name.includes(s) || email.includes(s);
                });
                            ...
                            {mode === "branch" ? (
                                ...
                                <div className="text-xs text-zinc-300 flex items-center gap-2">
                                    <span className="px-2 py-0.5 rounded bg-zinc-800 border border-zinc-700">
                                        Commit: <span className="font-mono">{commitSha.slice(0, 7)}</span>
                                    </span>
                                    {viewBranchOfCommit && (
                                        <span className="px-2 py-0.5 rounded bg-zinc-800 border border-zinc-700">
                                            from branch <b>{viewBranchOfCommit}</b>
                    ...
                    <Tabs value={tab} onValueChange={(v) => setTab(v as any)} className="flex-1 flex flex-col">
                        ...
                        <TabsContent value="code" className="flex-1 p-4 overflow-hidden">
                            <div className="mb-2 text-sm text-zinc-400 flex items-center flex-wrap gap-1">
                                <button className="hover:underline font-medium" onClick={() => setPath("")} title={repo?.full_name}>
                                ...
                                {path &&
                                    path.split("/").map((seg, i, arr) => (
                                        <React.Fragment key={i}>
                                            <span className="opacity-60">/</span>
                                            <button className="hover:underline" onClick={() => setPath(arr.slice(0, i + 1).join("/"))} title={arr.slice(0, i + 1).join("/")}>
                                                {seg}
                                            </button>
                                        </React.Fragment>
                                    ))}
                                    ...
                                    <ul className="divide-y divide-zinc-800">
                                        {items.map((c) => (
                                            <li key={c.path} className="flex items-center justify-between px-3 py-2 hover:bg-zinc-900">
                                                <button className="flex items-center gap-2 text-left" onClick={() => (c.type === "dir" ? setPath(c.path) : openCode(c.path))} title={c.path}>
                        ...
                        <TabsContent value="commits" className="flex-1 p-4 overflow-auto">
                            ...
                            >
                                            ...
                                            <div className="flex items-center justify-between gap-3 flex-wrap">
                                                <div className="min-w-0 flex items-center gap-3">
                                                    ...
                                                    <div className="min-w-0">
                                                        ...
                                                        <div className="text-xs text-zinc-400 truncate">
                                                            {c.author?.login && (
                                                                <a href={`https://github.com/${c.author.login}`} target="_blank" rel="noreferrer" className="hover:underline text-zinc-300">
                                ...
                                {!commitLoading && !commits.length && <li className="px-4 py-6 text-sm text-zinc-500">Không có commit nào.</li>}
                            </ul>
