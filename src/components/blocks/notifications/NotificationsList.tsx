...
import { Users, MessageCircle, Loader2, Settings, GitBranch, ChevronDown, Clock } from "lucide-react";
    ...
    switch (notification.type) {
      ...
      case "system":
        return {
          ...
          content: notification.data?.message || notification.data?.description || "Thông báo hệ thống",
      ...
      default:
        return {
          ...
          content: notification.data?.text || notification.data?.message || notification.data?.description || "Nội dung thông báo",
...
export default function NotificationsList({
  ...
  }, [hasMore, loading, notifications.length, onLoadMore, cleanupObserver, filter.page]);

  // Setup observer khi dependencies thay đổi
  useEffect(() => {
    if (filter.page >= 2) { // Chỉ setup từ trang 2
  ...
  const renderedNotifications = useMemo(() =>
    notifications.map((notification: any, index) => (
      ...
      />
    )), [notifications, selectedNotification?._id, handleNotificationSelect, formatTime, formatExactTime]
  );
