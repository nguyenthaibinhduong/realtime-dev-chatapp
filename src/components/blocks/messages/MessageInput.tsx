...
type Lang = "javascript" | "typescript" | "json" | "markdown" | "html" | "css" | "plaintext";
...
export const MessageInput = ({ channelId, onSend }: MessageInputProps) => {
    ...
    const canSend =
        (isCodeMode ? code.trim().length > 0 : newMessage.trim().length > 0) || files.length > 0;
    ...
    const handleSend = () => {
        ...
        if (isAutoCode) {
            const body = (isCodeMode ? code : newMessage.replace(/^```[^\n]*\n?|\n?```$/g, "")).trim();
    ...
    const canFormat = ["javascript", "typescript", "json", "html", "css", "markdown"].includes(lang);
    const formatCode = async () => {
        try {
            ...
            const formatted = await prettier.format(isCodeMode ? code : newMessage, {
                ...
                plugins: [pluginBabel, pluginEstree as any, pluginTS, pluginHTML, pluginMD, pluginCSS],
                tabWidth: 2,
                singleQuote: false,
                semi: true,
            });
            ...
            else setNewMessage("```" + (lang === "plaintext" ? "" : lang) + "\n" + formatted + "\n```");
        } catch (e) {
    ...
    return (
        <div className="border-t border-border p-2">
                ...
                <div className="flex items-center gap-1 py-2">
                    ...
                    <button
                        ...
                        onClick={() => {
                            ...
                            if (!isCodeMode && newMessage.trim()) setCode(newMessage.replace(/^```[^\n]*\n?|\n?```$/g, ""));
                            setIsCodeMode((v) => !v);
                        }}
                    ...
                    >
                        <Code2 className={`h-4 w-4 ${isAutoCode ? "text-white" : "text-muted-foreground"}`} />
                    </button>
                </div>
                        ...
                        {isAutoCode ? (
                            <>
                                ...
                                <div className="mb-2 flex items-center gap-2">
                                    ...
                                    <button
                                        ...
                                        title={canFormat ? "Format (Prettier)" : "Ngôn ngữ này chưa hỗ trợ format"}
                                    >
                                        Format
                                    </button>
                                </div>
                                ...
                                <div className="relative">
                                    <MonacoEditor
                                        height={220}
                                        language={lang === "plaintext" ? "plaintext" : (lang as any)}
                                        theme="vs-dark"
                                        value={code}
                            ...
                            <>
                                <textarea
                                    ...
                                    placeholder={isAutoCode ? "Soạn thảo code..." : `Nhắn tin đến #${channelId}...`}
                    ...
                    {previews.map((url, idx) => (
                        <div key={idx} className="relative">
                            {files[idx].type.startsWith("image") ? (
                                <img src={url} alt="preview" className="h-16 w-16 rounded border object-cover" />
                            ) : (
            ...
            <p className="mt-2 text-xs text-muted-foreground">
                Tip: dùng <kbd className="rounded bg-muted px-1 py-0.5 text-[10px]">```</kbd> hoặc{" "}
                <Code2 className="inline h-4 w-4 align-text-bottom" /> để bật soạn thảo code. Nhấn{" "}
                <kbd className="rounded bg-muted px-1 py-0.5 text-[10px]">Shift</kbd> +{" "}
                <kbd className="rounded bg-muted px-1 py-0.5 text-[10px]">Enter</kbd> để xuống dòng.
            </p>
        </div>
    );
};
